...
public class RedisClientTemplate {
     ...
     * @param key
     * @param value
     * @param dbIndex 数据库角标 0开始
     * @return
     */
    public String setIntoSpecifyDatabase(String key, String value, int dbIndex) {
        String result = null;
        ...
        Iterator<Jedis> jedis = allShards.iterator();
        while (jedis.hasNext()) {
            jedis.next().select(dbIndex);//选择第dbIndex+1个库 来存储
        }
        boolean broken = false;
        try {
            result = shardedJedis.set(key, value);
